...
class AuthService {
  async registration(email: string, password: string): Promise<AuthResponse__Output> {
    ...
    if (customerResponse?.rows?.[0]) {
      const customerInstance: Customer__Output = new CustomerDTO(customerResponse?.rows?.[0]);
